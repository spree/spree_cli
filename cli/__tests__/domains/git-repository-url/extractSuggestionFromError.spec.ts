...
describe('extractSuggestionFromError | unit tests', () => {
  it('extracts suggested git repository URL', async () => {
    const error = new git.Errors.UnknownTransportError(
      'git@github.com:upsidelab/test-store.git',
      'ssh',
      'https://github.com/upsidelab/test-store.git'
    );

    (validateGitRepositoryURL as ValidateGitRepositoryURL).mockResolvedValueOnce([true, null]);

    expect(await extractSuggestionFromError(error)).toBe('https://github.com/upsidelab/test-store.git');
  });

  it('returns \'null\' when there\'s no suggestion', async () => {
    const errorA = new git.Errors.UnknownTransportError('git@github.com:upsidelab/test-store.git', 'ssh');

    expect(await extractSuggestionFromError(errorA)).toBeNull();

    const errorB = new git.Errors.UrlParseError('https://github.com/upsidelab-template.git');
  ...
  it('checks if suggestion is valid before returning it', async () => {
    ...
    expect(await extractSuggestionFromError(errorB)).toBe('https://github.com/upsidelab/test-store.git');
  });
});
