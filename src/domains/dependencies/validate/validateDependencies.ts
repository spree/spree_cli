...
const validateDependenciesForModule = async (module: Module | BootModule) => {
  ...
  for (const [name, versionString] of Object.entries(dependencies)) {
    CliUx.ux.action.start(t('command.new_app.message.dependency_checking', { name, expectedVersion: versionString}));
    ...
    if (result.status === 'OK') {
      ...
      CliUx.ux.action.stop(color.red(t('command.new_app.message.error')));
      throw new Error(t('command.new_app.message.dependency_not_found', { name }));
    } else if (result.status === 'VERSION_MISMATCH') {
      CliUx.ux.action.stop(color.red(t('command.new_app.message.error')));
      throw new Error(t('command.new_app.message.dependency_version_mismatch', { name, expectedVersion: versionString, currentVersion: result.versionFound}));
    }
  }
};
